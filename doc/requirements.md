...
Endless Slice 是一个 `action` 类型的移动端小游戏：Swipe to slice the farm — pig, cow, chicken, sheep heads arc up; drag through to split them open. Chain hits for combos (×2, ×3, MASSIVE). Wagyu +100. Hit the no-cleaver sign and you're out.

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #ffd24a、#fff、#fffaf2、#b81818、#fff5e8、#fff0e0、#FFD700、rgba(0, 0, 0, 0.55)；高亮元素用于可点击目标、得分、结果或稀有状态。
- 字体：使用 'Baloo 2', system-ui, -apple-system, sans-serif、'Playfair Display', 'Bodoni 72', 'Didot', Georgia, serif、'Inter', system-ui, -apple-system, sans-serif、'Rye', 'Playfair Display', 'Bodoni 72', Georgia, serif，按钮与状态文字保持 12-24 px 的可读范围。
...
- 初始化参数：
- `LIVES`：3
- `COMBO_BONUS_PER_EXTRA`：10
- `GOLDEN_POINTS`：100
- `TRAIL_LIFE_MS`：200
- `FLASH_LIFE_MS`：260
- `TIER_CALLOUT_LIFE_MS`：850
- `SLICE_INTENT_DIST_SQ`：200
